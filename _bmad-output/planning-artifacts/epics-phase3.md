...
- **New finding (2026-07-24, from Story 11.3's own audit):** the "all controllers with `FileInterceptor`" source line undersold the actual surface — a full-codebase grep for `FileInterceptor`/`FilesInterceptor` turns up **two more upload endpoints this epic never documented**: `UsersController.uploadAvatar()` (`users.controller.ts:51-86`, `POST /users/profile/avatar`) already has its own inline MIME/size check (2 MB, jpeg/png/webp) and is already wired to real R2 — pure validation-logic duplication, not urgent. `MessagesController.uploadAttachment()` (`messages.controller.ts:57-64`, `POST /messages/attachments`) is a **complete untouched stub** — no `StorageService` injected at all, zero validation, still returns `{ url: 'https://placeholder/attachment' }`. Story 11.3 deliberately does **not** touch either of these (see its Dev Notes for rationale); tracked as a new backlog item — see `11-4-messages-attachment-upload-r2` in sprint-status.yaml.
